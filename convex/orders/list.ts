...
/**
 * Internal query to get all orders with child names.
 * Used by listAllOrdersForAdmin action.
 */
export const listAllOrdersWithChildNames = internalQuery({
  args: {},
  returns: v.array(
    v.object({
      _id: v.id("orders"),
  ...
  handler: async (ctx) => {
    // Get all orders from the database (no parentId filter)
